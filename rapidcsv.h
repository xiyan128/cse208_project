...
namespace rapidcsv
{
    ...
    /**
     * @brief     Exception thrown when attempting to access Document knn_data in a datatype which
    ...
    struct LabelParams
    {
         ...
         *                                to all rows as document knn_data. Default: 0
         ...
         *                                to all columns as document knn_data. Default: -1
         */
    ...
    /**
     * @brief     Datastructure holding parameters controlling how the CSV knn_data fields are separated.
     */
    struct SeparatorParams
    {
         ...
         * @param   pAutoQuote            specifies whether to automatically dequote knn_data during read, and add
        ...
        /**
         * @brief   specifies whether to automatically dequote cell knn_data.
         */
        bool mAutoQuote;
    ...
    public:
        ...
        /**
         * @brief   Constructor
         * @param   pStream               specifies a binary input stream to read CSV knn_data from.
         * @param   pLabelParams          specifies which row and column should be treated as labels.
         * @param   pSeparatorParams      specifies which field and row separators should be used.
         * @param   pConverterParams      specifies how invalid numbers (including empty strings) should be
         *                                handled.
        ...
        /**
         * @brief   Read Document knn_data from file.
         * @param   pPath                 specifies the path of an existing CSV-file to populate the Document
         *                                knn_data with.
         * @param   pLabelParams          specifies which row and column should be treated as labels.
         * @param   pSeparatorParams      specifies which field and row separators should be used.
        ...
        /**
         * @brief   Read Document knn_data from stream.
         * @param   pStream               specifies a binary input stream to read CSV knn_data from.
         * @param   pLabelParams          specifies which row and column should be treated as labels.
         * @param   pSeparatorParams      specifies which field and row separators should be used.
         * @param   pConverterParams      specifies how invalid numbers (including empty strings) should be
        ...
        /**
         * @brief   Write Document knn_data to file.
         ...
         *                                loading the Document knn_data will be used).
         */
        ...
        /**
         * @brief   Write Document knn_data to stream.
         * @param   pStream               specifies a binary output stream to write the knn_data to.
         */
        ...
        /**
         * @brief   Clears loaded Document knn_data.
         *
         */
        void Clear()
        {
      ...
      mIsLE = false;
         ...
         * @returns vector of column knn_data.
         */
        template<typename T>
         ...
         * @param   pColumnIdx            zero-based column index.
         * @param   pColumn               vector of column knn_data.
         */
        template<typename T>
         ...
         * @param   pColumnIdx            zero-based column index.
         * @param   pColumn               vector of column knn_data (optional argument).
        ...
        /**
         * @brief   Get number of knn_data columns (excluding label columns).
         ...
         * @returns vector of row knn_data.
         */
        template<typename T>
         ...
         * @param   pRowIdx               zero-based row index.
         * @param   pRow                  vector of row knn_data.
         */
        template<typename T>
         ...
         * @param   pRowIdx               zero-based row index.
         * @param   pRow                  vector of row knn_data (optional argument).
        ...
        /**
         * @brief   Get number of knn_data rows (excluding label rows).
         * @returns row count.
         */
         ...
         */
        void SetRowName(const size_t pRowIdx, const std::string& pRowName)
        {
            const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
      ...
      if ((bom2b == bomU16le) || (bom2b == bomU16be))
      {
        ...
        void ParseCsv(std::istream& pStream, std::streamsize p_FileLength)
        {
            ...
            while (p_FileLength > 0)
            {
                ...
                // knn_data length mismatch, so ensure we don't parse outside actual knn_data length read.
            ...
            if (mIsUtf16)
      {
        ...
        std::string Trim(const std::string& pStr) const
        {
            if (mSeparatorParams.mTrim)
            {
                ...
                // ltrim
                str.erase(str.begin(), std::find_if(str.begin(), str.end(), [](const int ch) { return !isspace(ch); }));

                // rtrim
                str.erase(std::find_if(str.rbegin(), str.rend(), [](const int ch) { return !isspace(ch); }).base(), str.end());
